...
class Database
{
    ...
    public static function updateDates($f_id, $kino, $dvd)
    {
		$stmt = Database::getInstance()->dbh->prepare("UPDATE `films` SET `kino_date` = :kino, `dvd_date` = :dvd WHERE `id` = :f_id");
    	$stmt->bindParam(':kino', $kino);
    	$stmt->bindParam(':dvd', $dvd);
    	$stmt->bindParam(':f_id', $f_id);
		if ($stmt->execute())
			return TRUE;
		else
